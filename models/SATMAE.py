...
from .pos_emb import get_2d_sincos_pos_embed, get_1d_sincos_pos_embed_from_grid


class GroupChannelsVisionTransformer(timm.models.vision_transformer.VisionTransformer):
    """ Vision Transformer with support for global average pooling
    """
    def __init__(self, global_pool=False, spectral_pool=True, channel_embed=256,
                 channel_groups=(tuple(range(0,50)), tuple(range(50, 100)), tuple(range(100, 150)), tuple(range(150, 202))), **kwargs):
    ...
    def get_intermediate_layers(self, x, norm = True):
        b, c, h, w = x.shape
        x_c_embed = []
        ...
        for blk in self.blocks:
            x = blk(x)
        if self.spectral_pool:
            if norm:
               x= self.norm(x)
            spectral_tokens = x[:, 1:, :]  # (N, G*L, D)
            spectral_tokens = spectral_tokens.view(b, G, L, D)  # (N, G, L, D)
            outcome = spectral_tokens.mean(dim=1)  # (N, L, D)
        else:
            if self.global_pool:
